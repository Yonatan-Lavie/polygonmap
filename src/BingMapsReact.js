...
export default function BingMapsReact({
  ...
  // removes poligons
  function removePoligons(map, Maps) {
    for (var i = map.entities.getLength() - 1; i >= 0; i--) {
      var poligon = map.entities.get(i);

      if (poligon instanceof Maps.Polygon) {
        map.entities.removeAt(i);
      }
    }
  }
  ...
  function addPoligons(PoligonPointsToAdd, map, Maps) {
    removePoligons(map, Maps);

    let listOfPoints = [];

    PoligonPointsToAdd.forEach((ponit) => {
      if (ponit === null) {
        return;
      }
      let newPoint = new Maps.Location(ponit.latitude, ponit.longitude);
      listOfPoints.push(newPoint);
    });

    let newPoligon = new Maps.Polygon(listOfPoints, null);

    map.entities.push(newPoligon);
  }
  // set view options
  ...
  const makeMap = useCallback(() => {
    ...
    if (polygonPoints) {
      addPoligons(polygonPoints, map.current, Maps);
    }
